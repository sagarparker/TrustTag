...
class Transaction extends _Executable.default {
    ...
    for (let i = 0; i < nodeIds.length; i++) {
      ...
      if (signedTransaction.sigMap != null && signedTransaction.sigMap.sigPair != null) {
        for (const sigPair of signedTransaction.sigMap.sigPair) {
          transaction._signerPublicKeys.add(hex.encode(sigPair.pubKeyPrefix));

          transaction._publicKeys.push(_cryptography.PublicKey.fromBytes(sigPair.pubKeyPrefix));
  ...
  async _mapResponse(response, nodeId, request) {
    const transactionHash = await sha384.digest(request.signedTransactionBytes);
    const transactionId = this.transactionId;
