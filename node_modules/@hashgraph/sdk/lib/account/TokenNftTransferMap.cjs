...
class TokenNftTransferMap extends _ObjectMap.default {
  ...
  static _fromProtobuf(transfers) {
    ...
    for (const transfer of transfers) {
      const token = _TokenId.default._fromProtobuf(transfer.token);

      for (const aa of transfer.nftTransfers != null ? transfer.nftTransfers : []) {
        const sender = _AccountId.default._fromProtobuf(aa.senderAccountID);

        const recipient = _AccountId.default._fromProtobuf(aa.receiverAccountID);
